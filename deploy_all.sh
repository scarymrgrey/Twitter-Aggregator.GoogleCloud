...
gcloud deployment-manager deployments create fedex-twitter-deployment-new --config deployment.yaml
gcloud dataflow jobs run pubsub2bq --gcs-location gs://dataflow-templates/latest/PubSub_Subscription_to_BigQuery --staging-location gs://${BUCKET_NAME}/staging --parameters inputSubscription=projects/${PROJECT_NAME}/subscriptions/pubsub2bq,outputTableSpec=${PROJECT_NAME}:tweetsds.tweets
